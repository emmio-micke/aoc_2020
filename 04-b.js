...
// Fetching the data from a function so we can move it out of the way.
let passports = get_data()
    // Split the passport data into individual array items.
    .split("\n\n")
    // Remove any extra line breaks.
    .map(value => value.replaceAll("\n", " "))
    // First, split row: 'hgt:171cm iyr:2013 pid:214368857 hcl:#cfa07d byr:1986 eyr:2028 ecl:grn'
    // into an array: ['hgt:171cm','iyr:2013','pid:214368857','hcl:#cfa07d','byr:1986','eyr:2028','ecl:grn']
    // Then, split each array value (string) into a key/value array: [ ['hgt','171cm'],['iyr','2013'] etc]
    .map(row => row.split(" ").map(value => value.split(":")))
    // Turn the new array into a Map so we can use it to check for keys.
    .map(value => new Map(value))
    // Filter out the passports that don't have the necessary fields. (Which are all fields exept cid.)
    .filter(passport => 
        // Convert byr to number and check.
        Number(passport.get('byr')) >= 1920 &&
        Number(passport.get('byr')) <= 2002 && 

        // Convert iyr to number and check.
        Number(passport.get('iyr')) >= 2010 &&
        Number(passport.get('iyr')) <= 2020 && 

        // Convert eyr to number and check.
        Number(passport.get('eyr')) >= 2020 &&
        Number(passport.get('eyr')) <= 2030 &&

        // Check that hgt exists.
        passport.has('hgt') &&
        (
            (
                // If string ends with cm, check cm requirements
                passport.get('hgt').slice(-2) == 'cm' &&
                parseInt(passport.get('hgt')) >= 150 &&
                parseInt(passport.get('hgt')) <= 193
            ) ||
            (
                // Or if string ends with in, check in requirements
                passport.get('hgt').slice(-2) == 'in' &&
                parseInt(passport.get('hgt')) >= 59 &&
                parseInt(passport.get('hgt')) <= 76
            )
        ) &&

        // Check if hcl starts with # followed by exactly 6 hexadecimal numbers and nothing else.
        RegExp(/^#[0-9a-fA-F]{6}$/).test(passport.get('hcl')) &&

        // Check that ecl is exactly one of the allowed values.
        ['amb','blu','brn','gry','grn','hzl','oth'].includes(passport.get('ecl')) &&

        // Check that pid consists of exactly 9 digits.
