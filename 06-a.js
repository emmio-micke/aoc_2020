...
let answers = get_data()
    ...
    .split("\n\n")
    // Remove all new lines within a group to get a single line string with all answers.
    .map(row => row.replaceAll("\n", ""))
    // Turn string into a Set and concat the Set into a string to get rid of all duplicates.
    .map(row => String.prototype.concat(...new Set(row)))
    // Count number of chars in each answer and return the combined sum.
    .reduce((acc, row) => acc + row.length, 0);

console.log(answers); // 6551
