...
let answers = get_data()
    // Split on double new line to get an array item for each group.
    .split("\n\n")
    // Split on new line to get an array item for each answer within the group.
    .map(row => row.split("\n"))
    // Check every answer in group for answers that are the same in every answer,
    // i e that everyone in the group has answered 'a', for instance.
    .map(group => {
        return group
            // Get the shortest answer from the group to minimize the loop.
            .reduce((a, b) => a.length <= b.length ? a : b)
            // Use filter to get a new array containing each array value that contains the current
            // char. If this array is as long as the whole array, the char is present in every
            // individual answer.
            .split("")
            .filter(char => {
                return group.length == group.filter(answer => answer.includes(char)).length;
            })
            .join("")
    })

    // Count number of chars in each answer and return the combined sum.
    .reduce((acc, row) => acc + row.length, 0);

console.log(answers); // 3358
